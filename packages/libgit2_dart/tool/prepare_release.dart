...
void main() async {
  ...
  const assetHashesPath =
      'packages/libgit2_dart/lib/src/hook/asset_hashes.dart';
  const pubspecPath = 'packages/libgit2_dart/pubspec.yaml';
  ...
  final tempDir = Directory.systemTemp.createTempSync('libgit2_dart-');
  try {
    await _gh([
      'run',
      'download',
    ...
    final newContent = downloaded.readAsStringSync().replaceFirst(
      ...
      "const releaseTag = 'libgit2_dart-v$version'",
    );
    ...
    print('Next steps:');
    print('  git tag libgit2_dart-v$version');
    print('  git push origin main libgit2_dart-v$version');
  } finally {
